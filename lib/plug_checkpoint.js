...
elseif ARGV[1]  ~= redis.call("hget", KEYS[1], "__ca_version__") then \
...
exports.newInstance = function($, spec, cb) {
    try {
        ...
        var wrapMapResponse = function(cb0) {
            return function (err, value) {
                if (err) {
                    ...
                    try {
                        var result = {remove: [], add : []};
                        var isVersion = false;
                        var key = null;
                        value.forEach(function(x, i) {
                            if (i%2 === 0) {
                                key = x;
                                isVersion = (key === '__ca_version__');
                            } else {
                                var parsedX = JSON.parse(x);
                                if (isVersion) {
                                    result.version = parsedX;
                                }
                                result.add.push(key);
        ...
        that.createMap = function(mapName, cb0) {
            ...
            that.__ca_doLuaOp__('createMap', [MAP_PREFIX + mapName], toAdd,
                                wrapMapResponse(cb0));
        };
        ...
        that.deleteMap = function(mapName, cb0) {
            that.__ca_doLuaOp__('deleteMap', [MAP_PREFIX + mapName], [], cb0);
        };
        ...
        that.readMap = function(mapName, cb0) {
            that.__ca_doLuaOp__('readMap', [MAP_PREFIX + mapName], [],
                                wrapMapResponse(cb0));
        };
        ...
        that.updateMap = function(mapName, changes, cb0) {
            ...
            that.__ca_doLuaOp__('updateMap', [MAP_PREFIX + mapName], args, cb0);
        };
        ...
        that.subscribeMap = function(mapName, handler, cb0) {
            var mapFull = MAP_PREFIX + mapName;
                ...
                that.__ca_getClientRedis__()
                    .on('message',  function(channel, message) {
                        ...
                        if (h) {
                            ...
                            $._.$.log && $._.$.log.debug('No handler for ' +
                                                         message);
                        }
                    });
                handlerTable = {};
