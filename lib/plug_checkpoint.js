    ...
    try {
        ...
        that.grabNodeLease = function(allPublicNodeIds, privateNodeId,
            ...
            if (allPublicNodeIds.length > 8000) {
                ...
                that.__ca_doLuaOp__('grabNodeLease', allPublicNodeIds,
                                    [privateNodeId, leaseTimeout], cb0);
            }
        };
         ...
         *
         * @param {Array.<string>} allPublicNodeIds  A list of
         * public nodeIds that we are interested on.
         * @param {function(Error, Object.<string,string|null>)} cb0 A
         * callback with an error or an object with external nodeIds to
         *  private nodeIds (or null if not in use).
         *
                ...
                var cb1 = function(err, data) {
                    if (err) {
                        ...
                        data.forEach(function(x, i) {
                                        result[allPublicNodeIds[i]] = (x ? x :
                                                                       null);
                                     });
                        cb0(err, result);
                    }
                };
                doLua('listNodes', allPublicNodeIds, [], cb1);
            }
        };
